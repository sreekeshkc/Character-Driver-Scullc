...
ssize_t scullc_read (struct file *filp, char __user *buf, size_t count,
                loff_t *f_pos)
{
	printk(KERN_CRIT"this is in scullc_read\n");
...
ssize_t scullc_write1 (struct file *filp,const char __user *buf, size_t count,
                loff_t *f_pos)
{
/*

#remove printk statements while writing large files(in mb).it vl produce oops mesgs due to log buffer overflow.
#writting 32768 bytes in each while loop wthout passing control to kernal,for updating write count.

*/	

//	printk(KERN_CRIT "*buf:%\t buf:%p \n",*buf,buf);

//	printk(KERN_CRIT "this is in write1\n");
//	printk(KERN_CRIT "count :%d\n",count );

///////////////////////////////////////////////////////////////////////////////////////////////

	size_t temp_count=count,n=count;
...
//		printk(KERN_CRIT "temp_count:%d\n",temp_count);
//		printk(KERN_CRIT "*f_pos :%d\n",*f_pos);
...
//		printk(KERN_CRIT "q_pos :%d\n",q_pos);
//		printk(KERN_CRIT "s_pos :%d\n",s_pos);
...
//			printk(KERN_CRIT "is in last loop\n");
//			printk(KERN_CRIT "temp_count:%d\n",temp_count);
			
			if (copy_from_user (dptr->data[s_pos]+q_pos, buf, temp_count))
               		{
                        	retval = -EFAULT;
                        	goto nomem;
               		}
...
//	printk(KERN_CRIT "@end*buf:%s \t buf:%p \n",*buf,buf);
...
ssize_t scullc_write (struct file *filp, const char __user *buf, size_t count,
                loff_t *f_pos)
{
	printk(KERN_CRIT "this is in write\n");
...
{
	 printk(KERN_CRIT "this is in asn write\n");
...
static ssize_t scullc_aio_write(struct kiocb *iocb, const struct iovec *iovec,
				unsigned long nr_segs, loff_t pos)
{
	 printk(KERN_CRIT "this is in aio_write\n");
